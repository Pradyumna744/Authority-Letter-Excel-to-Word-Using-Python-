...
from tkinter import Tk, Label, Button, filedialog, messagebox, Text, Scrollbar, ttk
...
def mail_merge(template_path, excel_path, output_folder, log_text, progress_bar):
    try:
        ...
        messagebox.showinfo("Success", "Mail merge completed!")
...
def start_merge():
    ...
    if not template_path or not excel_path or not output_folder:
        messagebox.showwarning("Missing Input", "Please provide all required inputs.")
        return
...
# Input Fields
ttk.Label(root, text="Word Template:").grid(row=0, column=0, sticky="w", padx=10, pady=5)
template_entry = ttk.Entry(root, width=50)
template_entry.grid(row=0, column=1, padx=10)
ttk.Button(root, text="Browse", command=select_template).grid(row=0, column=2, padx=10)

ttk.Label(root, text="Excel File:").grid(row=1, column=0, sticky="w", padx=10, pady=5)
excel_entry = ttk.Entry(root, width=50)
excel_entry.grid(row=1, column=1, padx=10)
ttk.Button(root, text="Browse", command=select_excel).grid(row=1, column=2, padx=10)

ttk.Label(root, text="Output Folder:").grid(row=2, column=0, sticky="w", padx=10, pady=5)
output_entry = ttk.Entry(root, width=50)
output_entry.grid(row=2, column=1, padx=10)
ttk.Button(root, text="Browse", command=select_output_folder).grid(row=2, column=2, padx=10)
...
log_label.grid(row=4, column=0, sticky="nw", padx=10)
log_text = Text(root, height=15, width=80, wrap="word", state="normal")
...
# Start Button
ttk.Button(root, text="Start Mail Merge", command=start_merge).grid(row=6, column=1, pady=20)
